    ...
    def label( self, data ):
        ...
        for state in temp_pratial_probablity:
            ...
            if temp_pratial_probablity[state] > count_max_prob:
                ...
                temp_state = state
        result_label_list.append(temp_state)

        ## count remaining left time state value with Viterbi
        for index in range(1, length_of_evidence):
            probability_now = dict()
            for state in self.states:
                max_prob_temp = -1
                for prev_state in self.states:
                    ## calculate the maximux possibility from previous state
                    previous_possibility = temp_pratial_probablity[prev_state]
                    transition_possibility = self.transitions[prev_state][state]
                    probability = previous_possibility * transition_possibility * self.getEmissionProb(state, data[index])
                    if probability > max_prob_temp:
                        max_prob_temp = probability
                probability_now[state] = max_prob_temp
            count_max_prob = -1
            temp_state = ''
            for state in probability_now:
                if probability_now[state] > count_max_prob:
                    count_max_prob = probability_now[state]
                    temp_state = state
            result_label_list.append(temp_state)
            ## use the probability in this time stamp for calculation in next time stamp
            temp_pratial_probablity = deepcopy(probability_now)
...
class StrokeLabeler:
    ...
    def confusion(self, trueLabels, classifications):
        '''
        Use Confusion matrix to evaluate the performance of the classifier

        Input:
            trueLabels: list of string representating the truth of the classifier string
            classifications: list of string representating the result of classification
        Output:
            dictionary representating the performance: number of corrected classified
        '''
        counting_table = dict()
        for outer_states in self.labels:
            counting_table[outer_states] = dict()
            for inner_states in self.labels:
                counting_table[outer_states][inner_states] = 0
        length_of_result = len(classifications)
        for index in range(length_of_result):
            if classifications[index] == trueLabels[index]:
                # classified correctly
                counting_table[trueLabels[index]][trueLabels[index]] += 1
            else:
                # classified Wrong
                counting_table[trueLabels[index]][classifications[index]] += 1
    ...
    def featurefy( self, strokes ):
        ...
        for s in strokes:
            ...
            ## add 3 continus feature here, defined in the file below
...
class Stroke:
    ...
    def bounding_box_ratio(self):
        '''
        Feature: hight/width ratio of the bunding box for the points
        '''
        left_margin_point = min(self.points, key = lambda x: x[0])
        right_margin_point = max(self.points, key = lambda x: x[0])
        top_margin_point = max(self.points, key = lambda x: x[1])
